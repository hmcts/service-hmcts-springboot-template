...
#   - Replace slug from `api-cp-springboot-template` to one of two (first in first used):
#      - user input
...
declare -a files_with_port=(.env Dockerfile README.md src/main/resources/application.yaml charts/rpe-api-cp-springboot-template/values.yaml)
declare -a files_with_slug=(build.gradle docker-compose.yml Dockerfile README.md ./infrastructure/main.tf ./src/main/java/uk/gov/hmcts/reform/demo/controllers/RootController.java charts/rpe-api-cp-springboot-template/Chart.yaml)
...
# Replace image repo
for i in "charts/rpe-api-cp-springboot-template/values.yaml"
do
  perl -i -pe "s/rpe/$product_name/g" ${i}
  perl -i -pe "s/api-cp-springboot-template/$component_name/g" ${i}
done

# Remove "rpe-" prefix from chart name to prepare it for api-cp-springboot-template slug replacement and update maintainer name
for i in "charts/rpe-api-cp-springboot-template/Chart.yaml"
do
  perl -i -pe "s/rpe-//g" ${i}
  perl -i -pe "s/rpe/$product_name/g" ${i}
done
...
# Replace api-cp-springboot-template slug
for i in ${files_with_slug[@]}
do
  perl -i -pe "s/api-cp-springboot-template/$slug/g" ${i}
done
...
git mv charts/rpe-api-cp-springboot-template charts/${slug}
